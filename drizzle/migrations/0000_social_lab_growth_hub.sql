...
-- ---------------------------------------------------------------------------
-- Data API grants (required for PostgREST access; RLS above still applies)
-- ---------------------------------------------------------------------------
do $$
declare t text;
begin
  foreach t in array array[
    'team_members', 'settings', 'list_items', 'daily_entries', 'daily_checkins', 'clients',
    'client_hours', 'leads', 'lead_activities', 'staff', 'location_defaults', 'packages',
    'meta_ads_daily', 'integration_runs', 'integration_state', 'checklist_items'
  ] loop
...
grant all on public.clients_with_stats to service_role;
